    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        public static void AddOrUpdateOverlay(GameUnit citizen)
        {
            try
            {
                ...
                byte[] imageData = File.ReadAllBytes(overlayPath);
                ...
                var srRenderer = sr as Renderer;

                var sortingGroupIDProp = typeof(Renderer).GetProperty("sortingGroupID", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                var sortingGroupOrderProp = typeof(Renderer).GetProperty("sortingGroupOrder", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);

                if (sortingGroupIDProp != null && sortingGroupOrderProp != null)
                {
                    // These values should match what worked in RUE
                    sortingGroupIDProp.SetValue(srRenderer, 25);
                    sortingGroupOrderProp.SetValue(srRenderer, 1);
                }
                else
                {
                    Plugin.Logger.LogWarning("Could not find sortingGroupID or sortingGroupOrder via reflection.");
                }
